...
def readLibrariesWithLdd(elfPath):
    ...
    :return:
    """
    proc = subprocess.Popen([C.LDD, elfPath], stdout=subprocess.PIPE)
    ...
    for lib in stdout.split('\n\t'):
        ...
        if 'linux-vdso' not in lib and 'ld-linux' not in lib:
            try:
                ...
                logging.critical("Parsing Error with %s outcome!" % (C.LDD))
...
def extractImportedFunctions(fileName, logger):
