...
namespace {
	...
	const std::vector<string> listOfLibs {"greetMorning","greetEvening","greetNight"} ;
	const std::vector<string> listOfLibs1 {"mmap64" , "mmap" , //"mprotect" , 
			"open", 
			"open64", "access", "write", "execve" , "fstat", "read", "pread" , "read64", 
			"clone", "pread64", "fopen", "fwrite" , "fread", "fseek",
			"read" , "write", "close", "stat", "lstat", "poll", "lseek", "munmap", 
			"brk", "rt_sigaction", "rt_sigprocmask", "rt_sigreturn", "ioctl", "readv",
			"writev", "pipe", "select", "sched_yeild", "mremap", "msync", "mincore",
			...
			"listen", "getsockname", "getpeername", "socketpair", "getsockopt",
			"setsockopt", /*"fork", "vfork",*/ /* "exit",*/ "wait4", "kill", "uname", "semget",
			"semop", "semctl", "shmdt", "msgget", "msgsnd", "msgrcv", "msgctl", "fnctl",
			"flock", "fsync", "fdatasync", "truncate", "ftruncate", "getdents", "getcwd",
			"chdir", "fchdir", "rename", "mkdir", "rmdir", "creat", "link", "unlink",
			"symlink", "readink", "chmod", "fchmod", "chown", "fchown", "lchown", "umask",
...
"vmsplice", "move_pages", "utimensat", "epoll_pwait", "ignalfd",
...
"userfaultfd", "mlock2", "copy_file_range", "preadv2", "pwritev2"
 };
		...
		for(Module::FunctionListType::iterator it = functions.begin(), it_end=functions.end(); it!=it_end; ++it){
            ...
            if(CI->getCalledFunction()!=NULL){
							...
							if (isInTestedLibs(fn_name)){// ==  "fwrite"){
								...
								SmallVector<Constant*, 16> knownConstExprArgs;
								...
								for(auto argCI = CI->arg_begin(); argCI != CI->arg_end(); ++argCI) {
									...
									if(auto* cin = dyn_cast<ConstantInt>(argCI)){
										...
										errs() << argCI->get()->getType() << "\n";
										errs() << cin->getValue() << "\n";
										errs() << llvm::ConstantInt::get(argCI->get()->getType(), cin->getValue());
									...
									//else if(auto* cin = dyn_cast<ConstantPointerNull>(argCI)){
										...
										errs() << "Type:: " << argCI->get()->getType() << "\n" ;
										knownConstExprArgs.push_back(llvm::ConstantInt::get(IntegerType::get(M.getContext(), 64), inn+1));
										knownConstExprArgs.push_back(cin);
									}
									else{
								...
								auto argsFromConstExpr = knownConstExprArgs.begin();
								...
								while(argsFromFn!=newArgs.end() || argsFromConst!=knownArgs.end() || argsFromConstExpr!=knownConstExprArgs.end()){
									...
									//}
									else if((argsFromConstExpr!=knownConstExprArgs.end() ) && (*argsFromConstExpr == llvm::ConstantInt::get(IntegerType::get(M.getContext(), 64), in))){
										argsFromConstExpr++;
